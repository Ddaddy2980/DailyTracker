...
function calcDayStatuses(
  ...
  for (let i = 0; i < durationDays; i++) {
    ...
    if (date > today)       { result[date] = 'future'; continue }
    if (date === today)     { result[date] = 'today'; continue }
...
function detectPatternAlertDay(
  ...
  for (let wd = 0; wd < 7; wd++) {
    ...
    for (let i = 1; i <= 21 && occurrences.length < 3; i++) {
      ...
      if (d.getDay() === wd) occurrences.push(new Intl.DateTimeFormat('en-CA').format(d))
    }
...
export default async function SoloingPage() {
  ...
  // Level-4 users with an active Soloing challenge belong here.
  // Level-4 users with an active Grooving challenge redirect to /grooving.
  if (challenge?.level === 4) {
    // continue — this is the right page
  } else if (challenge?.level === 3) {
    redirect('/grooving')
  } else if (!challenge) {
    redirect('/soloing/onboarding')
  } else {
    redirect('/soloing/onboarding')
  }

  if (!challenge) redirect('/soloing/onboarding')   // type narrowing
