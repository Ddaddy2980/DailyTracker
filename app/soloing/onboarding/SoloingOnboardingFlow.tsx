...
import { PILLAR_DISPLAY_NAMES } from '@/lib/next-pillar-invitation'
...
export default function SoloingOnboardingFlow({ allPillars, existingGoals, pillarLevels }: Props) {
  void pillarLevels // available for future per-pillar display
        ...
        <div className="w-full max-w-sm space-y-6 text-center">
          ...
          <div className="space-y-3">
            ...
            <h1 className="text-2xl font-black text-[var(--text-primary)] leading-snug">
              You&apos;re not building this habit anymore.
              <br />
              You&apos;re living it.
            </h1>
            <p className="text-[var(--text-secondary)] text-sm leading-relaxed">
              Soloing is where the scaffolding comes off. No pulse check.
              No coaching prompts. Just you, your pillars, and the discipline
  ...
  // ── Screen 2 — What Soloing Looks Like ────────────────────────────────────
  if (screen === 2) {
    const features = [
      {
        icon: '🎯',
        title: 'Up to 4 duration goals per pillar',
        body: 'You can layer complexity into each pillar — not just one goal, but a full stack.',
      },
      {
        icon: '🏔️',
        title: 'Unlimited destination goals',
        body: 'Keep pointing toward something bigger. Your destination goals have no cap.',
      },
      {
        icon: '📅',
        title: '90 or 100-day challenge',
        body: 'A longer runway gives your habits more time to become something you don\'t question.',
      },
    ]
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12"
        ...
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="text-center space-y-2">
            <p className="text-xs font-bold uppercase tracking-widest text-violet-600">
              What Soloing looks like
            </p>
            <h2 className="text-xl font-black text-[var(--text-primary)]">
              Same daily rhythm. More depth.
            </h2>
          </div>
  ...
  ] satisfies { days: DurationDays; label: string; sub: string }[]
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          {allPillars.map(p => (
            <span
              key={p}
              className={`text-xs font-bold uppercase tracking-wide ${PILLAR_COLOR[p] ?? 'text-[var(--text-muted)]'}`}
            >
              {PILLAR_DISPLAY_NAMES[p as keyof typeof PILLAR_DISPLAY_NAMES] ?? p}
            </span>
          ))}
        </div>
