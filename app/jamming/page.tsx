...
export default async function JammingPage() {
  ...
  if (challenge.level !== 2) redirect('/challenge')

  const today        = todayStr()
  const durationDays = challenge.duration_days

  const [entries, earnedRewards, pendingPulse, pulseHistory, watchedVideoIds, myGroups] = await Promise.all([
  ...
  // Day number (1-indexed, capped at durationDays)
